...
export default function IntegrationsTab() {
  ...
  const fetchers = connections.filter(
    (c) => allServices.find((s) => s.serviceType === c.serviceType)?.isFetcher,
  );
  const uploaders = allServices.filter((s) => s.isUploader);

  const invalidateStatus = () =>
    queryClient.invalidateQueries({
      queryKey: getGetApiConnectionsStatusQueryKey(),
    });
  ...
  const addMutation = usePostApiCredentials({
    mutation: {
      onSuccess: () => {
        ...
        setEditing(null);
        queryClient.invalidateQueries({
          queryKey: getGetApiConnectionsQueryKey(),
        });
        queryClient.invalidateQueries({
  ...
  const disconnectMutation = useDeleteApiConnectionsServiceType({
    mutation: {
      onSuccess: () => {
        ...
        queryClient.invalidateQueries({
          queryKey: getGetApiCredentialsAvailableQueryKey(),
        });
        invalidateStatus();
      },
    },
  ...
  const mappingMutation = usePutApiConnectionsMappings({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: getGetApiConnectionsMappingsQueryKey(),
        });
        invalidateStatus();
      },
    },
  ...
  const getMappedDests = (src: string) =>
    mappings.find((m) => m.sourceServiceType === src)
  ...
  const handleToggle = (
    ...
    mappingMutation.mutate({
      data: { sourceServiceType: fetcherType, destinationServiceTypes: next },
    });
  };
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
        ...
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {fetchers.map((f) => (
            <FetcherDestinationCard
              ...
              onToggle={(dest, enabled) =>
                handleToggle(f.serviceType ?? "", dest, enabled)
              }
            />
          ))}
          {fetchers.length === 0 && (
      ...
      <CredentialModal
        ...
        onSubmit={(data: CreateCredentialRequest) =>
