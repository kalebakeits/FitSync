...
export default function PublishToDateModal({
  open,
  initialDate,
  onClose,
}: Props) {
  ...
  const { data: workoutsData } = useGetApiWorkouts(
    { limit: 200, offset: 0 },
    {
      query: { enabled: open },
    },
      ...
      >
        {destinations.length === 0 ? (
          <Typography variant="body2" color="text.secondary">
            No supported destinations connected. Connect Wahoo or Garmin in the
            Integrations tab.
          </Typography>
        ) : (
          <>
        ...
        <Button
          ...
          onClick={() =>
            publishMutation.mutate({
              workoutId,
              data: { serviceType, scheduledDate },
            })
          }
        >
          Schedule
