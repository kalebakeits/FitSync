...
function avg(low: number, high?: number) {
...
function intensityFraction(step: Step): number {
  ...
  if (step.targetZone != null) {
    if (isPower)
      return Math.min(POWER_ZONES[step.targetZone - 1] ?? 0.5, 1.5) / 1.5;
...
function StepBar({ step }: { step: Step }) {
  ...
  const height = Math.max(
    MIN_HEIGHT,
    Math.round(intensityFraction(step) * MAX_HEIGHT),
  );

  return (
    <Tooltip
      title={[step.name ?? label, duration, target].filter(Boolean).join(" · ")}
      arrow
    >
      <Box
        sx={{
...
function flattenToSteps(items: WorkoutItem[]): Step[] {
  ...
  for (const item of items) {
    if (item.kind === "repeat") {
      const inner = ((item as RepeatItem).steps ?? []).filter(
        (s): s is Step => s.kind === "step" || s.kind === "swimStep",
      );
...
function totalDurationMs(steps: Step[]): number {
  return steps.reduce((sum, s) => {
    if (s.durationType === 0 && s.durationValue != null)
      return sum + s.durationValue;
    return sum;
  }, 0);
}
