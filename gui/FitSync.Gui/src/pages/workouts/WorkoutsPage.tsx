...
export default function WorkoutsPage() {
  ...
  const [deleteWorkout, setDeleteWorkout] = useState<WorkoutResponse | null>(
    null,
  );
  const [publishWorkout, setPublishWorkout] = useState<WorkoutResponse | null>(
    null,
  );

  const { data, isLoading, error } = useGetApiWorkouts({
  ...
  return (
    <Box sx={{ p: 3, width: "100%" }}>
      ...
      <DeleteWorkoutModal
        ...
        onConfirm={() =>
          deleteWorkout?.id && deleteMutation.mutate({ id: deleteWorkout.id })
        }
