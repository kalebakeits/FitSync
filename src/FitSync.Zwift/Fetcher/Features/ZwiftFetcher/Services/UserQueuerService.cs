...
) : IUserQueuerService
{
    ...
    public async Task<User[]> GetDueUsersAsync()
    {
        ...
        this.logger.LogInformation("Claimed {Count} Zwift users.", userIds.Length);
    ...
    public async Task<bool> ReleaseUsersAsync(User[] users)
    {
        ...
        if (claimed.Count == 0)
        {
            this.logger.LogWarning("Worker {InstanceId} found no Zwift configs to release.", instanceId);
            return false;
        }
        ...
        this.logger.LogInformation("Released {Count} Zwift user configurations.", released);
        return released > 0;
    }
}
